...
class nice_mail {
	...
	function nice_mail($config, $is_html = false) {
		$this->mail_method = $config;
		
		$this->from = "no-reply@elbook.su";
		$this->charset = "utf-8";
		$this->site_name = "Elbook";

		
		$this->smtp_host = '';
		$this->smtp_port = intval( '' );
		$this->smtp_user = '';
		$this->smtp_pass = '';
	...
	function compile_headers() {
		...
		$this->mail_headers .= "Priority: 3" . $this->eol;
		$this->mail_headers .= "NSMail-Priority: Normal" . $this->eol;
		$this->mail_headers .= "Mailer: NiceMail path" . $this->eol;
